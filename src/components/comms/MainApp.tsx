...
export default function MainApp({ user, org, onSignOut, onSwitchOrg, embedded = false, showProfileButton = false }: Props) {
  ...
  const [filterTab, setFilterTab] = useState<"chats" | "unread" | "groups">("chats")
  ...
  const filteredThreads = threads.filter(t => {
    if (filterTab === "unread" && !unreadCounts[t.id]) return false
  ...
  visibleThreads.sort((left, right) => {
    ...
    if (leftIsTom === rightIsTom) return 0
    return leftIsTom ? -1 : 1
  })
  ...
  function subscribeToCallStatus(callId: string, threadId?: string) {
    ...
    const unsub = onSnapshot(doc(firestore, "comms_v5_calls", callId), async snap => {
      ...
      if (data.status === "declined" || data.status === "missed") {
        if (threadId) {
          await postCallMessage(threadId, false, 0)
        }
        cleanupCall()
        return
      }
  ...
  async function endCall() {
    ...
    if (activeCall.calleeUid === TOM_UID || activeCall.callerUid === TOM_UID) {
      if (callThreadIdRef.current) {
        await postCallMessage(callThreadIdRef.current, duration > 0, duration)
      }
      cleanupCall()
      return
    }
  ...
  async function postCallMessage(threadId: string, answered: boolean, duration: number) {
  ...
  return (
    <div
      ref={appRef}
      className={`relative flex flex-col overflow-hidden bg-[#e8f6fd] ${embedded ? "h-full max-w-none" : "max-w-md mx-auto"}`}
    ...
    >
      ...
      {/* ── Header ── */}
      <div
        className="bg-[#e8f6fd] px-5 pb-4 shrink-0"
      ...
      >
        ...
        <div className="mb-4 mt-2 flex items-center gap-2 text-sm text-[#0891b2]/70">
          ...
          {groupLabel ? <span className="text-[#67c2d8]">|</span> : null}
          {groupLabel ? <span>{groupLabel}</span> : null}
        </div>

        {permissionWarning ? (
          <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] leading-5 text-amber-800">
            {permissionWarning}
          </div>
        ) : null}

        {/* Search bar */}
        <div className="flex items-center gap-3 bg-[#d4eef8] rounded-2xl px-4 py-3 border border-sky-200/60">
          <Search size={16} className="text-[#0891b2] shrink-0" />
          <input
            value={searchQuery}
      ...
      {/* ── Filter row ── */}
      <div className="bg-[#e8f6fd] border-b border-sky-100 px-4 py-3 flex items-center gap-3 shrink-0">
        {/* Contacts icon */}
        ...
        <div className="flex gap-2 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {(["chats","unread","groups"] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setFilterTab(tab)}
              className={`px-4 py-1.5 rounded-full text-sm transition-colors ${
                filterTab === tab
                  ? "bg-[#29b6d8] text-white"
                  : "text-gray-500 hover:text-gray-700"
              }`}
            >
              {tab === "chats" ? "Chats" : tab === "unread" ? "Unread" : "Groups"}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* ── Thread list ── */}
      <div className="flex-1 overflow-y-auto bg-[#e8f6fd]">
        {visibleThreads.length === 0 && (
          <p className="text-center text-gray-400 text-sm mt-20">No conversations yet</p>
        )}
        ...
        {visibleThreads.map(thread => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 px-5 py-4 border-b border-sky-100/40 active:bg-sky-200/30 ${
                selectedThread?.id === thread.id ? "bg-sky-200/40" : ""
              }`}
            >
              {/* Avatar */}
              ...
              <div className="flex-1 min-w-0 text-left">
                <div className="flex items-center justify-between mb-0.5">
                  <span className={`text-[15px] truncate ${unread ? "font-semibold text-[#0c4a6e]" : "text-[#0e7490]"}`}>
                    {name}
                  </span>
                  <span className="text-xs text-[#67c2d8] shrink-0 ml-2">
                    {thread.updatedAt ? formatTime(thread.updatedAt) : ""}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#67c2d8] truncate">
                    {thread.type === "channel" ? "Group" : thread.lastMessage}
                  </span>
                  {unread > 0 && (
                    <span className="ml-2 min-w-[22px] h-[22px] bg-[#29b6d8] rounded-full text-white text-xs flex items-center justify-center px-1.5 shrink-0">
                      {unread > 99 ? "99+" : unread}
                    </span>
                  )}
                </div>
      ...
      {/* ═══════════════════ THREAD VIEW ═══════════════════ */}
      {selectedThread && (
        <div className="absolute inset-0 flex flex-col bg-[#e8f6fd] z-10">
          {/* Thread header */}
          <div
            className="bg-[#e8f6fd] border-b border-sky-100 px-5 pb-4 flex items-center gap-3 shrink-0"
            style={{ paddingTop: "calc(env(safe-area-inset-top) + 16px)" }}
          >
            <button onClick={() => setSelectedThread(null)} className="mr-1">
              <ArrowLeft size={22} className="text-[#0891b2]" />
            </button>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[#0c4a6e] text-base truncate">{getThreadName(selectedThread)}</p>
              {selectedThread.type === "direct" && (
                <p className="text-[#0891b2]/60 text-sm">
              ...
              {selectedThread.type === "channel" && selectedThread.description && (
                <p className="text-[#0891b2]/60 text-sm truncate">{selectedThread.description}</p>
              )}
            </div>
            {selectedThread.type === "direct" && (
              <button onClick={() => initiateCall(getOtherUid(selectedThread), selectedThread.id)}
            ...
            {messages.map((msg, idx) => {
              ...
              if (isSystem) return (
                <div key={msg.id} className="flex justify-center my-3">
                  <span className="bg-white text-gray-400 text-sm px-4 py-1.5 rounded-full shadow-sm">{msg.text}</span>
                </div>
              )
              if (msg.deleted) return (
                <div key={msg.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                  <span className="text-sm text-gray-400 italic px-3 py-1">This message was deleted</span>
                </div>
              )
                ...
                >
                  ...
                  <div className={`flex flex-col ${isOwn ? "items-end" : "items-start"} max-w-[84%]`}>
                    {!isOwn && showSenderName && (
                      <span className="text-sm text-[#0891b2] mb-1 ml-1">{msg.displayName}</span>
                    )}

                    {msg.replyTo && (
                      <div className={`text-sm text-gray-400 bg-gray-100 rounded-t-xl px-3 py-2 border-l-2 border-[#29b6d8] mb-0.5 max-w-full ${isOwn ? "rounded-bl-xl" : "rounded-br-xl"}`}>
                    ...
                    {editingMessage?.id === msg.id ? (
                      <div className="flex items-center gap-2 w-full">
                        <input
                          ...
                          className="flex-1 bg-white border border-[#29b6d8] rounded-xl px-3 py-2 text-[16px] outline-none"
                        />
                      ...
                      <button
                        ...
                        isOwn ? "bg-gradient-to-br from-[#29b6d8] to-[#1a86c8] text-white rounded-br-sm" : "bg-white text-[#0e7490] rounded-bl-sm shadow-sm"
                      }`}>
                        ...
                        {msg.text}
                        {msg.edited && <span className={`ml-1 text-xs ${isOwn ? "text-white/50" : "text-gray-400"}`}>(edited)</span>}
                      </button>
                    )}
                          ...
                          uids.length > 0 && (
                            <button key={emoji} onClick={() => toggleReaction(msg.id, emoji)}
                              className={`flex items-center gap-1 rounded-full border px-2.5 py-1 text-sm leading-none shadow-sm ${
                                uids.includes(user.uid) ? "border-sky-300 bg-sky-100 text-sky-700" : "border-gray-200 bg-white text-gray-600"
                              }`}>
                              {emoji} {uids.length}
                            </button>
                          )
                    ...
                    <span className="text-xs text-[#67c2d8] mt-1 mx-1">{formatTime(msg.createdAt)}</span>
                  </div>
            ...
            {(otherIsTyping || showTomTyping) && (
              <div className="flex items-end gap-0.5">
                ...
                <div className="flex max-w-[84%] flex-col items-start">
                  <div className="rounded-2xl rounded-bl-sm bg-white px-4 py-3 text-left text-[15px] leading-relaxed text-[#0e7490] shadow-sm">
          ...
          {/* Reply banner */}
          {replyTo && (
            <div className="flex items-center gap-3 px-4 py-2.5 bg-sky-50 border-t border-sky-100 shrink-0">
              <div className="flex-1 min-w-0">
          ...
          {/* Input */}
          <div
            className="px-4 py-3 bg-[#e8f6fd] border-t border-sky-100 shrink-0 relative"
            style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 16px)" }}
          >
            <div className="flex items-center gap-3 rounded-2xl border border-sky-200/60 bg-[#d4eef8] px-4 py-3 pr-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.35)]">
              <button onClick={() => fileInputRef.current?.click()} className="text-[#0891b2] shrink-0">
                <Paperclip size={18} />
              </button>
              ...
              <input
                ...
                placeholder="Message…"
                className="min-w-0 flex-1 bg-transparent text-[16px] text-gray-700 placeholder-gray-500 outline-none"
              />
