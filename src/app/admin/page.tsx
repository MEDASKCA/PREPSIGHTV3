...
export default function AdminPage() {
  ...
  async function approveMembership(membershipId: string) {
    const approved = await approveOrganizationMembership(uid, membershipId)
    if (!approved) {
      push("Membership approval failed", "err")
      return
    }
    push("Membership approved")
    loadRegistrations()
  }
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {membership.status === "pending_approval" ? (
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
                                    >
